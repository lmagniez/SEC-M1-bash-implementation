...
%token CMD AND PIPE OR BACKGROUND SEPARATOR FLUX_WRITE FLUX_READ MY_JOBS EXIT MY_LS END BLANK SET SET_DISPLAY UNSET SET_ENV SET_ENV_DISPLAY UNSET_ENV
...
%left SET SET_DISPLAY UNSET SET_ENV SET_ENV_DISPLAY UNSET_ENV
...
Command:
	CMD {addToStack($1);}
	|SET_ENV{addToStack($1); addToStack(OP_SET_ENV);}
	|SET_ENV_DISPLAY{addToStack(OP_SET_ENV_DISPLAY);}
	|UNSET_ENV{addToStack(OP_UNSET_ENV);}
	|SET {addToStack($1); addToStack(OP_SET);}
	|SET_DISPLAY {addToStack(OP_SET_DISPLAY);}
	|UNSET {addToStack($1); addToStack(OP_UNSET);}
